...
平台：毓数；页面标题：显示“数据质量告警记录”。

导航模式：自定义菜单；顶部菜单入口：数据资产；当前菜单路径：风险监控 > 数据质量监控 > 告警记录。

一级菜单：
- 数据地图，图标 Icon/yushu/shujuditu，无子菜单
- 工作流管理，图标 Icon/yushu/flow-manage，有子菜单
- 数据表管理，图标 Icon/yushu/shujubiao，无子菜单
- 风险监控，图标 Icon/yushu/zichananquan，有子菜单
...
筛选字段：告警名称、所属项目、告警级别、处置状态、发生时间；显示形式：带标题筛选项；触发方式：按钮触发。

列表操作栏：左侧“批量确认、批量关闭”，右侧次要操作“刷新、导出”；主操作：“新建质量规则”，位置：列表最右侧。
...
当前菜单路径只用于生成和选中左侧菜单层级，不在内容区域生成面包屑。不要添加组件库中不存在或需求中未说明的组件和内容。
